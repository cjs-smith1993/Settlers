...
public class MapComponent extends JComponent
{
	...
	public MapComponent()
	{
		...
		this.hexes = new HashMap<HexLocation, HexType>();
		this.roads = new HashMap<EdgeLocation, CatanColor>();
		this.settlements = new HashMap<VertexLocation, CatanColor>();
		this.cities = new HashMap<VertexLocation, CatanColor>();
		this.ports = new HashMap<EdgeLocation, PortType>();
		this.numbers = new HashMap<HexLocation, Integer>();
		this.robber = null;
		this.allHexPoints = new HashMap<HexLocation, Point2D>();
		this.allVertexPoints = new HashMap<VertexLocation, Point2D>();
		this.allEdgePoints = new HashMap<EdgeLocation, Point2D>();
	...
	private void initDrop()
	{
		...
		this.dropHexLoc = null;
		this.dropEdgeLoc = null;
		this.dropVertLoc = null;
	}
	...
	public void addHex(HexLocation hexLoc, HexType hexType)
	{
		...
		// Add hex to hex map
		this.hexes.put(hexLoc, hexType);

		// Compute hex point for the new hex
		this.allHexPoints.put(hexLoc, getHexPoint(hexLoc));
		...
		for (EdgeDirection edgeDir : EdgeDirection.values())
		{
			...
			this.allEdgePoints.put(edgeLoc, getEdgePoint(edgeLoc));
		}
		...
		for (VertexDirection vertDir : VertexDirection.values())
		{
			...
			this.allVertexPoints.put(vertLoc, getVertexPoint(vertLoc));
		}
	...
	public void placeRoad(EdgeLocation edgeLoc, CatanColor color)
	{
		...
		this.roads.put(edgeLoc.getNormalizedLocation(), color);
	...
	public void placeSettlement(VertexLocation vertLoc, CatanColor color)
	{
		...
		if (this.cities.containsKey(normVertLoc))
		{
			this.cities.remove(normVertLoc);
		}

		this.settlements.put(normVertLoc, color);
	}
	...
	public void placeCity(VertexLocation vertLoc, CatanColor color)
	{
		...
		if (this.settlements.containsKey(normVertLoc))
		{
			this.settlements.remove(normVertLoc);
		}

		this.cities.put(normVertLoc, color);
	}

	public void placePort(EdgeLocation edgeLoc, PortType portType)
	{
		this.ports.put(edgeLoc, portType);
	}
	...
	public void startDrop(PieceType pieceType, CatanColor pieceColor)
	{
		...
		this.dropType = pieceType;
		this.dropColor = pieceColor;
	...
	private MouseAdapter mouseAdapter = new MouseAdapter() {
		...
		public void mouseMoved(MouseEvent e)
		{
			...
			if (MapComponent.this.dropType == null)
			{
				return;
			}
			if (MapComponent.this.transform == null)
			{
				return;
			}
			...
			try
			{
				MapComponent.this.transform.inverseTransform(mousePoint, mousePoint);
			}
			...
			if (MapComponent.this.dropType == PieceType.ROAD)
			{

				EdgeLocation closestEdgeLoc = null;
				double closestDistance = 0;

				for (Map.Entry<EdgeLocation, Point2D> entry : MapComponent.this.allEdgePoints
						.entrySet())
				{
				...
				MapComponent.this.dropEdgeLoc = closestEdgeLoc;
				MapComponent.this.dropAllowed = MapComponent.this.getController().canPlaceRoad(
						MapComponent.this.dropEdgeLoc);
			}
			else if (MapComponent.this.dropType == PieceType.CITY
					|| MapComponent.this.dropType == PieceType.SETTLEMENT)
			{

				VertexLocation closestVertLoc = null;
				double closestDistance = 0;

				for (Map.Entry<VertexLocation, Point2D> entry : MapComponent.this.allVertexPoints
						.entrySet())
				{
				...
				MapComponent.this.dropVertLoc = closestVertLoc;

				if (MapComponent.this.dropType == PieceType.CITY)
				{
					MapComponent.this.dropAllowed = MapComponent.this.getController().canPlaceCity(
							MapComponent.this.dropVertLoc);
				}
				else if (MapComponent.this.dropType == PieceType.SETTLEMENT)
				{
					MapComponent.this.dropAllowed = MapComponent.this.getController()
							.canPlaceSettlement(MapComponent.this.dropVertLoc);
				}
				else
				{
					assert false;
			...
			else if (MapComponent.this.dropType == PieceType.ROBBER)
			{

				HexLocation closestHexLoc = null;
				double closestDistance = 0;

				for (Map.Entry<HexLocation, Point2D> entry : MapComponent.this.allHexPoints
						.entrySet())
				{
				...
				MapComponent.this.dropHexLoc = closestHexLoc;
				MapComponent.this.dropAllowed = MapComponent.this.getController().canPlaceRobber(
						MapComponent.this.dropHexLoc);
			}
			else
			{
				assert false;
		...
		public void mouseClicked(MouseEvent e)
		{
			...
			if (MapComponent.this.dropType != null)
			{
				...
				if (MapComponent.this.dropAllowed)
				{
					switch (MapComponent.this.dropType)
					{
					case ROAD:
						MapComponent.this.getController().placeRoad(MapComponent.this.dropEdgeLoc);
						break;
					case SETTLEMENT:
						MapComponent.this.getController().placeSettlement(
								MapComponent.this.dropVertLoc);
						break;
					case CITY:
						MapComponent.this.getController().placeCity(MapComponent.this.dropVertLoc);
						break;
					case ROBBER:
						MapComponent.this.getController().placeRobber(MapComponent.this.dropHexLoc);
						break;
					default:
						assert false;
						break;
	...
	protected void paintComponent(Graphics g)
	{
		...
		this.drawRobber(g2);
		this.drawRoads(g2);
		this.drawSettlements(g2);
		this.drawCities(g2);
		this.drawDropShape(g2);
	}

	private void drawHexes(Graphics2D g2)
	{

		for (Map.Entry<HexLocation, HexType> entry : this.hexes.entrySet())
		{
	...
	private void drawNumbers(Graphics2D g2)
	{
		...
		for (Map.Entry<HexLocation, Integer> entry : this.numbers.entrySet())
		{
			...
			this.drawImage(g2, numImage, hexCenter);
		}
	}
	...
	private void drawRobber(Graphics2D g2)
	{
		if (this.robber != null)
		{
			this.drawRobber(g2, this.robber);
		}
	}
	...
	private void drawRobber(Graphics2D g2, HexLocation hexLoc)
	{
		...
		this.drawImage(g2, robberImage, hexPoint);
	}

	private void drawRoads(Graphics2D g2)
	{
		for (Map.Entry<EdgeLocation, CatanColor> entry : this.roads.entrySet())
		{
			EdgeLocation edgeLoc = entry.getKey();
			CatanColor color = entry.getValue();
			this.drawRoad(g2, edgeLoc, color);
		}
	}
	...
	private void drawSettlements(Graphics2D g2)
	{
		...
		for (Map.Entry<VertexLocation, CatanColor> entry : this.settlements.entrySet())
		{
			VertexLocation vertLoc = entry.getKey();
			CatanColor color = entry.getValue();
			this.drawSettlement(g2, vertLoc, color);
		}
	}
	...
	private void drawCities(Graphics2D g2)
	{
		...
		for (Map.Entry<VertexLocation, CatanColor> entry : this.cities.entrySet())
		{
			VertexLocation vertLoc = entry.getKey();
			CatanColor color = entry.getValue();
			this.drawCity(g2, vertLoc, color);
		}
	}
	...
	{
		for (Map.Entry<EdgeLocation, PortType> entry : this.ports.entrySet())
		{
			EdgeLocation edgeLoc = entry.getKey();
			PortType portType = entry.getValue();
			this.drawPort(g2, edgeLoc, portType);
		}
	}
			...
			drawPort(Graphics2D g2, EdgeLocation edgeLoc, PortType portType)
	{
		...
		this.drawRotatedImage(g2, getPortImage(portType), imageLoc,
				this.getPortRotation(edgeLoc));
	}

	private void drawDisallowImage(Graphics2D g2, Point2D location)
	{
		this.drawImage(g2, getDisallowImage(), location);
	}
	...
	private void drawDropShape(Graphics2D g2)
	{
		...
		{
		case ROAD: {
			if (this.dropEdgeLoc != null)
			{
				if (this.dropAllowed)
				{
					this.drawRoad(g2, this.dropEdgeLoc, this.dropColor);
				}
				else
				{
					this.drawDisallowImage(g2, getEdgePoint(this.dropEdgeLoc));
				}
			}
		}
			break;
		case CITY: {
			if (this.dropVertLoc != null)
			{
				if (this.dropAllowed)
				{
					this.drawCity(g2, this.dropVertLoc, this.dropColor);
				}
				else
				{
					this.drawDisallowImage(g2, getVertexPoint(this.dropVertLoc));
				}
			}
		}
			break;
		case SETTLEMENT: {
			if (this.dropVertLoc != null)
			{
				if (this.dropAllowed)
				{
					this.drawSettlement(g2, this.dropVertLoc, this.dropColor);
				}
				else
				{
					this.drawDisallowImage(g2, getVertexPoint(this.dropVertLoc));
			...
			break;
		case ROBBER: {
			if (this.dropHexLoc != null)
			{
				if (this.dropAllowed)
				{
					this.drawRobber(g2, this.dropHexLoc);
				}
				else
				{
					this.drawDisallowImage(g2, getHexPoint(this.dropHexLoc));
				}
			}
		}
			break;
			...
			Point2D location, double radians)
	{
		...
		this.drawImage(g2, op.filter(image, null), location);
	}
